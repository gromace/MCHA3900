...
if __name__ == '__main__':
    import argparse
    commandParse = True # Assuming trying to run through argparse
    ...
    try:
        ...
        parser.add_argument('--video',
                    required=False,
                    metavar=r"<y/l/n>",
                    help="Run offline video (y), webcam video (live) or not (n)")
        
        parser.add_argument('--videopath',
                    required=False,
                    metavar=r"/path/to/file",
                    help="Run detection on specified file")
    ...
    # Nothing to parse
        ...
        video = booleanAll[0]
        live = booleanAll[1]
        video_path = "blackfly_duck_video.avi"

    # Want this after the exception, in case they make an error while trying
    # to use command line
    if commandParse is True:
        dataset = args.dataset.lower()
        weights = args.weights.lower()
        command = args.command.lower()
        video_path = args.videopath.lower()
    
        if args.video.lower == 'y':
            video = True
            live = False
        elif args.video.lower == 'live':
            video = True
            live = True
        else:
            video = False
    ...
    # Configurations
    if dataset == "duckie":
        DEFAULT_WEIGHTS_PATH = ROOT_DIR + "\DuckieDetection\duckieDetection.h5"
        ...
        DEFAULT_WEIGHTS_PATH = ROOT_DIR + "\RobotXDetection\RobotXDetection.h5"
